...
urlpatterns = [
    ...
    path('meus-pedidos/<int:pk>/', meu_pedido_detalhe, name='meu_pedido'),
    path('<int:variacao_id>/carrinho/', adicionar_ao_carrinho, name='adicionar_ao_carrinho'),
    path('<int:variacao_id>/comprar/', comprar_agora, name='comprar_agora'),
    path('<slug:slug>/', detalhe_produto_cliente, name='detalhe_produto_cliente'),
    path('checkout/endereco/', checkout_endereco, name='checkout_endereco'),
]